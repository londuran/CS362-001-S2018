   ...
   /**
	...
	//			System.out.println(" Seed:"+randomseed );
				Random random = new Random(randomseed);
				 ...
				 int startDay=ValuesGenerator.getRandomIntBetween(random, -1, 35);
			...
			for (int i = 0; i < NUM_TESTS; i++) {
					String methodName = ApptRandomTest.RandomSelectMethod(random);
					   if (methodName.equals("setValid")){
						   ...
						   int recurValue = ValuesGenerator.getRandomIntBetween(random, 0, 8);
						   if(recurValue == 7){
							   recurDays = null;
						   }
						   ...
						   int badDay=ValuesGenerator.getRandomIntBetween(random, 35, 65);
						   int badMonth=ValuesGenerator.getRandomIntBetween(random, 15, 35);
						   int badYear=ValuesGenerator.RandInt(random);
						   appt.isOn(badDay, badMonth, badYear);
					   }
